...
df %>% 
  ...
  filter(grepl("ave", Address, ignore.case = T)) %>%
...
```{r}
df %>% summarise_if(is.numeric, max, na.rm = T)
```
...
df %>% 
  mutate(
    CreatedDate_new = parse_date_time(
      CreatedDate, 
      orders = "%m%d%y %H%M%S %p" # this is how we tell it what order
                                  # the components come in
  )) %>%
...
```{r}
df2 %>%
  mutate(DaysOpen_new = difftime(CloseDate, CreatedDate, units = "days") %>% round(0)) %>%
